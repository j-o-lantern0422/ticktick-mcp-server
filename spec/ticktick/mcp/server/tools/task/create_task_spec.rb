...
RSpec.describe Ticktick::Mcp::Server::CreateTask do
  let(:client) { instance_double(Ticktick::Client) }

  before { allow(Ticktick::Client).to receive(:new).and_return(client) }

  it "returns created task as formatted JSON" do
    task = { "id" => "task_new", "title" => "Buy milk", "projectId" => "proj_001" }
    allow(client).to receive(:create_task)
      .with(title: "Buy milk", project_id: "proj_001", content: nil, desc: nil,
    ...
    parsed = JSON.parse(described_class.call(title: "Buy milk", project_id: "proj_001").content.first[:text])
    expect(parsed["title"]).to eq("Buy milk")
    expect(parsed["projectId"]).to eq("proj_001")
  end

  it "passes all optional parameters to the client" do
    task = { "id" => "task_new", "title" => "Meeting" }
    allow(client).to receive(:create_task)
      .with(
        title: "Meeting",
        project_id: "proj_001",
  ...
  it "returns error when token is missing" do
    ...
    response = described_class.call(title: "Test", project_id: "proj_001")
  ...
  it "returns error on API failure" do
    ...
    content = described_class.call(title: "Bad", project_id: "proj_001").content.first
