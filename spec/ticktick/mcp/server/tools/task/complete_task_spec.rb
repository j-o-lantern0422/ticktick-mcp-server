...
RSpec.describe Ticktick::Mcp::Server::CompleteTask do
  let(:client) { instance_double(Ticktick::Client) }

  before { allow(Ticktick::Client).to receive(:new).and_return(client) }

  it "returns success message when task is completed" do
    allow(client).to receive(:complete_task).with(project_id: "proj_001", task_id: "task_001").and_return(nil)

    response = described_class.call(project_id: "proj_001", task_id: "task_001")
    expect(response.content.first[:text]).to eq("Task task_001 marked as complete.")
  end
  ...
  it "returns error on API failure" do
    allow(client).to receive(:complete_task)
