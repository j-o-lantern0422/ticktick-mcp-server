...
RSpec.describe Ticktick::Mcp::Server::DeleteProject do
  let(:client) { instance_double(Ticktick::Client) }

  before { allow(Ticktick::Client).to receive(:new).and_return(client) }

  it "returns success message when project is deleted" do
    allow(client).to receive(:delete_project).with(project_id: "proj_001").and_return(nil)

    response = described_class.call(project_id: "proj_001")
    expect(response.content.first[:text]).to eq("Project proj_001 deleted.")
  end
